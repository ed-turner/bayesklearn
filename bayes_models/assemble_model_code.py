...
def assembleData( cat_cols, is_regression = True ):
    '''
    Inputs:
...
#effects
def assembleCategoryEffects( cat_cols ):
...
def assembleParameters( cat_cols, is_regression ):
    '''
    Inputs :
    ...
    if len(cat_cols) == 0:
        params = 'vector[p] beta; real<lower = 0> sigma; real shift; real mu_b; real<lower=0> sigma_b; real mu_s; real<lower=0> sigma_s; '
    ...
    sigmas = [ 'real<lower=0> sigma_' + cat_col[0] for cat_col in cat_cols ] + ['real<lower=0> sigma_b', 'real<lower=0> sigma']
...
def assembleModelCodeGaussianRegression(cat_cols):
...
def assembleModelCodeGaussianClassification( cat_cols ):
...
def assembleModelCode( cat_cols = [], is_regression = True ):

    data = assembleData( cat_cols, is_regression = is_regression )

    params = assembleParameters( cat_cols, is_regression )

    if is_regression:
        model = assembleModelCodeGaussianRegression( cat_cols )

    else:
        model = assembleModelCodeGaussianClassification( cat_cols )
